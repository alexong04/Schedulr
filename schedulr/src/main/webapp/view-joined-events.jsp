...
<head>
<meta charset="UTF-8">
<title>View Joined Events</title>
<style>
	body {
		margin: 0;
		padding: 0;
...
<body>
	<h1><%= session.getAttribute("currentUser") %>'s Joined Events</h1>
	<div class="events-container">
		<%
			...
			eventQuery = con.createStatement().executeQuery("SELECT * FROM events");
			
			while (eventQuery.next()){
				if (EventsHandler.isParticipant(eventQuery.getString(15), currentUID)) {
					...
					hostQuery = con.createStatement().executeQuery("SELECT username FROM users WHERE UID = " + eventQuery.getInt(2));
					if (hostQuery.next()) {
						eventHost = hostQuery.getString(1);
